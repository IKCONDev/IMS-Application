...
public class UsersWebSecurity {
	...
	/**
	 * the web security configurer adapter is deprecated , going forward for
	 * registering a security filter chain would be as shown below
	 * 
	 * @param http
	 * @return
	 * @throws Exception
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests().antMatchers("/users/**").permitAll().antMatchers(HttpMethod.OPTIONS, "/**").permitAll()
	...
	public UserAuthenticationFilter getAuthenticationFilter(HttpSecurity http) throws Exception {
		...
		UserAuthenticationFilter authenticationFilter = new UserAuthenticationFilter(service, environment,
				authenticationManager(http.getSharedObject(AuthenticationConfiguration.class)), encoder);
		// set authentication manager on your auth filter
